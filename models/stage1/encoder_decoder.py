    ...
    def __init__(
        ...
        **kwargs
        ...
        self.encoder = nn.Sequential(
            ...
            *[
                VQVAEEncBlock(d, d, dropout_rate)
                for _ in range(int(np.log2(downsample_rate)) - 1)
            ],
            *[
                nn.Sequential(
                    ResBlock(d, d, bn=bn, dropout_rate=dropout_rate), nn.BatchNorm2d(d)
                )
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.decoder = nn.Sequential(
            *[
                nn.Sequential(
                    ResBlock(d, d, dropout_rate=dropout_rate), nn.BatchNorm2d(d)
                )
                for _ in range(n_resnet_blocks)
            ],
            *[
                VQVAEDecBlock(d, d, dropout_rate=dropout_rate)
                for _ in range(int(np.log2(downsample_rate)) - 1)
            ],
            nn.ConvTranspose2d(
                d, num_channels, kernel_size=(3, 4), stride=(1, 2), padding=(1, 1)
            ),
            nn.ConvTranspose2d(
                num_channels,
                num_channels,
                ...
                stride=(1, 2),
                padding=(1, 1),
            ),  # one more upsampling layer is added not to miss reconstruction details
        )
